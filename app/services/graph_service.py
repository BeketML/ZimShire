"""Compiled LangGraph + Postgres checkpointer/store lifecycle.

Lifespan (in `app/main.py`) calls `init_graph()` once and `close_graph()` on
shutdown. The compiled graph and the underlying checkpointer/store are exposed
via `get_graph()` to routers.
"""
...
def _to_psycopg_dsn(url: str) -> str:
    """Strip SQLAlchemy driver suffix so psycopg can parse the URI."""
...
def get_graph():
    g = _state["graph"]
    if g is None:
        raise RuntimeError("Graph not initialised — call init_graph() in FastAPI lifespan")
    return g
...
def get_store():
    s = _state["store"]
    if s is None:
        raise RuntimeError("Store not initialised — call init_graph() in FastAPI lifespan")
    return s
